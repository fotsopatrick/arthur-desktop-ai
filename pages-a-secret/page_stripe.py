#!/usr/bin/env python3
"""Page fabriquee le 16/09/2026 par ~/outils/page-a-secret.py — service stripe."""
import json, os, urllib.error, urllib.request
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

PORT = 8802
SERVICE = 'stripe'
FICHE = {'nom': 'Stripe', 'quoi': 'le secret', 'a_quoi_ca_sert': 'me laisser parler a ce service en ton nom', 'debuts': [], 'longueur_mini': 12, 'exemple': 'colle la ligne entiere', 'essai': None, 'ou_le_fabriquer': ['Connecte-toi au service.', 'Cherche la page des clefs ou des jetons dans tes reglages.', 'Fabrique-en un, et copie la ligne entiere.'], 'couleurs': ('#0e1116', '#171c24', '#2c3543', '#e7ecf3', '#4a8cff')}
COFFRE = os.path.expanduser("~/.secrets/secret-stripe.txt")
...
<title>Le secret de Stripe</title><style>
...
if __name__ == "__main__":
    print("%s -> http://127.0.0.1:%d" % ('Le secret de Stripe', PORT), flush=True)
